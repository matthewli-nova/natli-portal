...
export function TicketTypesTable({ 
  ...
  const [editFormData, setEditFormData] = React.useState<Partial<TicketType>>({})
  ...
  const eventTickets = ticketTypes.filter(t => t.ticketScope === "event")
  const subEventTickets = ticketTypes.filter(t => t.ticketScope === "sub_event")

  const getTicketName = (ticket: TicketType) => {
    return ticket.nameEn || "Untitled Ticket"
  }

  const startEditTicket = (ticket: TicketType) => {
  ...
  const cancelEdit = () => {
    ...
    const ticket = ticketTypes.find(t => t.id === editingTicket)
    if (ticket && !ticket.nameEn && ticket.registeredCount === 0) {
      onTicketTypesChange(ticketTypes.filter(t => t.id !== editingTicket))
    }
  ...
  const saveTicket = () => {
    ...
    if (!editFormData.nameEn?.trim()) {
      toast.error("Ticket name (English) is required")
      return
    }
    ...
    // Validate sub-event must have parent
    if (editFormData.ticketScope === "sub_event" && !editFormData.parentTicketTypeId) {
      toast.error("Sub-event tickets must have a parent ticket type")
      return
    }

    const updatedTickets = ticketTypes.map(t => 
      t.id === editingTicket ? editFormData as TicketType : t
    )
    onTicketTypesChange(updatedTickets)
    setEditingTicket(null)
    setEditFormData({})
    setExpandedAdvanced(new Set())
    toast.success("Ticket type saved successfully")
  }
  ...
  const deleteTicket = (ticketId: string) => {
    ...
    if (ticket.registeredCount > 0) {
      toast.error(`Cannot delete — ${ticket.registeredCount} registrations use this ticket type.`)
      return
    }

    // Check if other tickets depend on this as parent
    const dependentTickets = ticketTypes.filter(t => t.parentTicketTypeId === ticketId)
    if (dependentTickets.length > 0) {
      toast.error(`Cannot delete — ${dependentTickets.length} sub-event ticket(s) depend on this ticket.`)
      return
    }
  ...
  const handleTicketScopeChange = (value: "event" | "sub_event") => {
    const updates: Partial<TicketType> = {
      ticketScope: value,
    }

    if (value === "sub_event") {
      // Lock access scope to custom for sub-events
      updates.accessScope = "custom"
      updates.parentTicketTypeId = eventTickets.length > 0 ? eventTickets[0].id : undefined
    } else {
      // Clear parent for event tickets
      updates.parentTicketTypeId = undefined
    }
  ...
  const renderTicketRow = (ticket: TicketType, isEditing: boolean) => {
    const parentTicket = ticket.parentTicketTypeId 
      ? ticketTypes.find(t => t.id === ticket.parentTicketTypeId)
      : null

    if (isEditing) {
      // Determine colspan based on ticket scope
      const colSpan = ticket.ticketScope === "event" ? 4 : 8
            ...
            <TableCell colSpan={colSpan} className="p-0">
              <div className="p-4 space-y-4">
                <div className="text-sm font-semibold mb-3">
                  {ticket.nameEn ? "Edit Ticket Type" : "New Ticket Type"}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <Tabs value={nameLang} onValueChange={setNameLang}>
                      ...
                      <TabsContent value="en" className="mt-2">
                        <Input
                          ...
                          placeholder="Enter ticket name in English"
                          maxLength={100}
                          className="h-9"
                        />
                  ...
                  <div className="space-y-2">
                    ...
                    <ColourPicker
                      value={editFormData.colorHex || "#4CAF50"}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    type="number"
                    min={isPublished ? editFormData.registeredCount || 0 : 0}
                  ...
                  {isPublished && editFormData.registeredCount! > 0 && (
                    <div className="text-xs text-orange-600">Min: {editFormData.registeredCount}</div>
                  )}
                </div>

                {/* Ticket Scope - only for sub-event tickets */}
                {ticket.ticketScope === "sub_event" && (
                  <div className="space-y-2">
                    <Label className="flex items-center gap-1 text-xs">
                      Ticket Scope <span className="text-destructive">*</span>
                    </Label>
                    <Select
                      value={editFormData.ticketScope || "event"}
                      onValueChange={handleTicketScopeChange}
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="sub_event">Sub-event</SelectItem>
                      </SelectContent>
                    </Select>
                    {isPublished && (
                      <div className="text-xs text-orange-600">Cannot change after publish</div>
                    )}
                  </div>
                )}

                {/* Parent Ticket - only for sub-event tickets */}
                {ticket.ticketScope === "sub_event" && (
                  <div className="space-y-2">
                    <Label className="flex items-center gap-1 text-xs">
                      Parent Ticket Type <span className="text-destructive">*</span>
                    </Label>
                    {eventTickets.length === 0 ? (
                      <div className="text-sm text-muted-foreground p-2 border rounded-md bg-muted/30">
                        Create an event-level ticket type first
                      </div>
                    ) : (
                      <Select
                        value={editFormData.parentTicketTypeId || ""}
                        onValueChange={(value) => setEditFormData({ ...editFormData, parentTicketTypeId: value })}
                        disabled={isPublished}
                      >
                          ...
                          {eventTickets.map((t) => (
                            <SelectItem key={t.id} value={t.id}>
                              <div className="flex items-center gap-2">
                                {renderColorSwatch(t.colorHex)}
                                <span>{getTicketName(t)}</span>
                              </div>
                            </SelectItem>
                          ))}
                        </SelectContent>
    ...
    // Display mode - render based on ticket scope
    if (ticket.ticketScope === "event") {
      // Event ticket - 4 columns
      return (
        <TableRow 
      ...
      // Sub-event ticket - 8 columns (original structure)
      return (
        <TableRow 
          className={cn(
            "hover:bg-muted/20",
        ...
        >
          ...
          <TableCell>
            {parentTicket ? (
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <span>↳</span>
                {renderColorSwatch(parentTicket.colorHex)}
                <span>{getTicketName(parentTicket)}</span>
              </div>
            ) : (
              <span className="text-sm text-muted-foreground">—</span>
            )}
          ...
          <TableCell className="text-center text-sm text-muted-foreground">
            {ticket.maxBringAlongDefault === null ? "Unlimited" : 
             ticket.maxBringAlongDefault === 0 ? "None" : 
             ticket.maxBringAlongDefault}
          </TableCell>
          <TableCell className="text-right">
            <div className="flex items-center justify-end gap-1">
              <Button
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Define ticket types for event registration
          </p>
        </div>
        <Button onClick={startAddTicket} size="sm" className="gap-2">
      ...
      <div className="space-y-3">
        <h4 className="text-sm font-semibold text-muted-foreground">Public Guest Tickets</h4>
        <div className="border rounded-lg overflow-hidden">
          <Table>
            ...
            <TableBody>
              {eventTickets.length === 0 && !editingTicket ? (
                <TableRow>
                  <TableCell colSpan={4} className="h-24 text-center text-muted-foreground">
                    No event tickets yet. Click "Add Ticket Type" to create one.
                  </TableCell>
                </TableRow>
              ) : (
                <>
                  {eventTickets.map((ticket) => renderTicketRow(ticket, editingTicket === ticket.id))}
                  {editingTicket && editFormData.ticketScope === "event" && 
                   !eventTickets.find(t => t.id === editingTicket) && 
                   renderTicketRow(editFormData as TicketType, true)}
                </>
              )}
            </TableBody>
          </Table>
      ...
      {/* Sub-event Tickets */}
      {(subEventTickets.length > 0 || (editingTicket && editFormData.ticketScope === "sub_event")) && (
        <div className="space-y-3">
          <h4 className="text-sm font-semibold text-muted-foreground">Sub-event Tickets</h4>
          <div className="border rounded-lg overflow-hidden">
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/30">
                  ...
                  <TableHead className="font-semibold">Name</TableHead>
                  <TableHead className="font-semibold w-[120px]">Scope</TableHead>
                  <TableHead className="font-semibold w-[140px]">Access Scope</TableHead>
                  <TableHead className="font-semibold w-[140px]">Parent Ticket</TableHead>
                  <TableHead className="font-semibold w-[140px] text-center">Registered</TableHead>
                  <TableHead className="font-semibold w-[100px] text-center">Companions</TableHead>
                  <TableHead className="font-semibold w-[100px] text-right">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {subEventTickets.length === 0 && editingTicket && editFormData.ticketScope === "sub_event" ? null : (
                  subEventTickets.map((ticket) => renderTicketRow(ticket, editingTicket === ticket.id))
                )}
                {editingTicket && editFormData.ticketScope === "sub_event" && 
                 !subEventTickets.find(t => t.id === editingTicket) && 
                 renderTicketRow(editFormData as TicketType, true)}
              </TableBody>
            </Table>
          </div>
