...
export function SessionDrawer({ session, onClose }: SessionDrawerProps) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed right-0 top-0 h-screen w-[520px] max-w-[90vw] z-50 flex flex-col bg-muted/50 shadow-2xl transform transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {session && (
          <>
            {/* Header */}
            ...
            <div className="overflow-y-auto flex-1 p-5 space-y-4">
              ...
              <div className="bg-card rounded-xl border border-border p-4 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${session.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground'}`}>
                    {session.isActive ? '● Active' : formatAge(session.ageMs)}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                  <span>Agent: <strong className="text-foreground">{session.agentId}</strong></span>
                  <span>Kind: <strong className="text-foreground">{session.kind}</strong></span>
                  <span>Model: <strong className="text-foreground">{getModelShortName(session.model)}</strong></span>
                  <span>Tokens: <strong className="text-foreground">{formatTokens(session.totalTokens || 0)}</strong></span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                  ...
                  {total > messages.length && <span className="ml-1 text-muted-foreground/70">of {total} total</span>}
                </h3>
              </div>
              ...
              {!loading && !fetchError && messages.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm font-medium text-foreground/80">No messages found</p>
                  <p className="text-xs text-muted-foreground mt-1">This session may not have a stored transcript</p>
                </div>
              )}
                ...
                msg.role === 'user' ? (
                  ...
                  <div key={msg.id} className="flex gap-2 items-start">
                    ...
                    <div className="max-w-[85%]">
                      ...
                      {msg.timestamp && (
                        <p className="text-[10px] text-muted-foreground mt-1 ml-1">{formatTime(msg.timestamp)}</p>
                      )}
                    </div>
                  </div>
                )
