...
import { PaymentProviderData, ProviderTableData, calculatePercentChange } from "./payment-mock-data";
import { formatCurrency, formatNumber } from "./mock-data";
import { ChangeIndicator } from "../ui/change-indicator";
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Legend, Cell } from "recharts";

export function PaymentProviderChart() {
  const [isReady, setIsReady] = useState(false);
  const totalAmount = PaymentProviderData.reduce((acc, curr) => acc + curr.current, 0);
  const totalPrevious = ProviderTableData.reduce((acc, curr) => acc + curr.previous, 0);
  const totalTransactions = ProviderTableData.reduce((acc, curr) => acc + curr.transactions, 0);
  ...
  return (
    <Card className="flex flex-col h-full gap-0 overflow-hidden">
      <CardHeader className="pt-[21px] pl-[21px] pr-[21px] pb-[7px]">
        <CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Sales by Provider</CardTitle>
      </CardHeader>
            ...
            <ResponsiveContainer width="100%" height="100%" minWidth={100} minHeight={100} debounce={50}>
              ...
              >
                ...
                <Bar dataKey="current" barSize={20} radius={[0, 4, 4, 0]}>
                  {PaymentProviderData.map((entry, index) => (
          ...
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent border-b bg-muted/50">
                <TableHead className="pl-[21px] text-foreground">Provider</TableHead>
            ...
            <TableBody>
              ...
              <TableRow className="bg-muted/50 font-bold hover:bg-muted/50 border-t-2">
                <TableCell className="pl-[21px]">TOTAL (Non-Cash)</TableCell>
                ...
                <TableCell className="text-right">{(totalAmount / 52340 * 100).toFixed(2)}%</TableCell>
                <TableCell className="text-right pr-[21px]">{totalTransactions}</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </div>
